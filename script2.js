var TicTacToe = {
    ...
    // Set the game mode to single-player
    ...
    // Updated init() function to always start in single-player mode
    init: function() {
        // ... (existing code)
        this.symbols = ["X", "O"];
        ...
        // Start a new game in single-player mode
        this.newGame();
    },
    ...
    // Updated play() function to include single-player mode logic
    play: function(el) {
        // ... (existing code)
        ...
        if (this.singlePlayerMode && this.activePlayer === 1 && !this.gameOver) {
            this.computerPlay();
        }
    },

    // New function for the computer's move in single-player mode
    computerPlay: function() {
        // Find an empty square randomly (you can implement a more sophisticated AI)
        var emptySquares = this.squares.filter(square => square.classList.length === 1);
        if (emptySquares.length > 0) {
            var randomIndex = Math.floor(Math.random() * emptySquares.length);
            this.play(emptySquares[randomIndex]);
        }
    },
